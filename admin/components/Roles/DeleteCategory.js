import { deleteRole, getAllRole } from "@/state/Admin/Action";
...
const DeleteRole = (props) => {
  ...
  const handleDelete = () => {
    dispatch(deleteRole(props.data)).then((value) => {
      dispatch(getAllRole());
  ...
  return (
    <div id="root">
      <div className="absolute w-2/6 px-10 py-5 mt-4 -translate-x-1/2 -translate-y-1/2 bg-white top-1/2 left-1/2 rounded-xl">
        <div className="font-semibold tracking-wide">Delete role</div>

        <div className="mt-5">Do you want to delete this role?</div>
