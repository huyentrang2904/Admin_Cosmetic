import { getAllUser, deleteAccount } from "@/state/Admin/Action";
...
const DeleteMember = (props) => {
  ...
  const handleDelete = () => {
    dispatch(deleteAccount(props.data)).then((value) => {
      dispatch(getAllUser())
  ...
  return (
    <div id="root">
      <div className="absolute w-2/6 px-10 py-5 mt-4 -translate-x-1/2 -translate-y-1/2 bg-white top-1/2 left-1/2 rounded-xl">
        <div className="font-semibold tracking-wide">Remove member</div>

        <div className="mt-5">Do you want to remove this member?</div>
